...
namespace SpareParts.AppService.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<IResult<GenericResponse>> SendVerificationEmail(
            string kindOf,
            string to,
            string subject,
            bool isHtmlBody,
        ...
        )
        {
            ...
            {
                body = EmailBody.GetUserVerificationBody(
                    to,
                    _configModel.SparePartsEmail.Sender,
                    token,
                    isForWebsite == true
                        ? _configModel.AppConfig.WebsiteUrl
                        : _configModel.AppConfig.DashboardUrl
                );
                successMsg =
                    $@"A verification email is sent to {to}. Please verify your email by following the instructions.";
            }
            else if (kindOf.Equals(ConstVariables.ResetPasswordEmailBody))
            {
                body = EmailBody.GetForgotPasswordBody(
                    _configModel.SparePartsEmail.Sender,
                    token,
                    isForWebsite == true
                        ? _configModel.AppConfig.WebsiteUrl + "/account"
                        : _configModel.AppConfig.DashboardUrl + "/auth"
                );
                successMsg =
                    $@"A password reset token is sent to {to}. Please follow the instructions to reset your password.";
            }
            else if (kindOf.Equals(ConstVariables.NewAccountConformationBody))
            {
                body = EmailBody.GetNewAccountConformationBody(
                    _configModel.SparePartsEmail.Sender,
                    token,
                    _configModel.AppConfig.DashboardUrl,
                    to
            ...
            else
            {
                if (kindOf.Equals(ConstVariables.NewAccountConformationBody))
                {
                    return Response<GenericResponse>.ErrorResponse(
                        @"Account has been created but failed to send email to the account holder."
                    );
                }
                return Response<GenericResponse>.ErrorResponse(
                    @"Problem Sending Email. Please resend the email."
                );
            }
        }

        public Task<HttpStatusCode> SendEmailWithAttachment(
            List<string> to,
            string subject,
            string body,
            bool isHtmlBody,
        ...
        )
        {
            var emailMessage = BuildEmailHeaders(
                _configModel.SparePartsEmail.Sender,
                _configModel.SparePartsEmail.Name,
                to,
                cc,
                bcc,
                subject
        ...
        private Task<HttpStatusCode> SendEmail(
            List<string> to,
            string subject,
            string body,
            bool isHtmlBody,
        ...
        private static MimeMessage BuildEmailHeaders(
            string from,
            string fromName,
            List<string> to,
        ...
        public async Task<HttpStatusCode> SendEmailSMTP(
            List<string> to,
            string subject,
            string body,
            bool isHtmlBody,
        ...
        )
        {
            ...
            using var client = new SmtpClient(_configModel.AwsSmtp.Host, _configModel.AwsSmtp.Port)
            {
                Credentials = new NetworkCredential(
                    _configModel.AwsSmtp.Username,
                    _configModel.AwsSmtp.Password
                ),
