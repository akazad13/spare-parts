...
namespace SpareParts.AppService.Services
{
    public class AccountService : Helper, IAccountService
    {
        ...
        public async Task<IResult<LogInSuccessDto>> LogInService(
            LoginDto loginData,
            bool fromWebsite
        )
        {
            ...
            {
                return await Response<LogInSuccessDto>.GetErrorResponseMessage(
                    "No Rrcord found with this email and password."
                );
            }
            ...
            if (
                (alwaysAUser.Role == Roles.Customer && !fromWebsite)
                || (alwaysAUser.Role != Roles.Customer && fromWebsite)
            )
            {
                return await Response<LogInSuccessDto>.GetErrorResponseMessage(
            ...
            {
                return await Response<LogInSuccessDto>.GetErrorResponseMessage(
                    "You account is deactive. Please contact the support."
                );
            }
            ...
            {
                return await Response<LogInSuccessDto>.GetErrorResponseMessage(
                    "You are not verified. Please confirm your email address."
                );
            }

            return VerifyPassword(loginData.Password, alwaysAUser.Password)
                ...
                : await Response<LogInSuccessDto>.GetErrorResponseMessage(
                    "No Rrcord found with this email and password."
                );
        }
        ...
        public Task<IResult<GenericResponse>> Register(
            IOption<long> id,
        ...
        )
        {
            ...
            var addressData = new UserAddress(
                ...
                userProfileDto.City,
                userProfileDto.State,
                userProfileDto.PostCode ?? 0,
        ...
        public async Task<IResult<GenericResponse>> VerifyCustomer(VerificationDto verificationData)
        {
            ...
            if (user.IsNone)
            {
                return await Response<GenericResponse>.GetErrorResponseMessage(
                    "No Rrcord found with this email address"
                );
            }
            ...
            if (
                !(
                    new Helper().TokenToHash(verificationData.VerificationCode)
                    == alwaysAUser.VerificationCode
                )
            )
            {
                return await Response<GenericResponse>.GetErrorResponseMessage(
            ...
            return _userService.UpdateUser(alwaysAUser, alwaysAUser.Id)
                ? await Task<IResult<GenericResponse>>.Factory.StartNew(
                    () => Response<GenericResponse>.SuccessResponese("User Is Verified")
                )
                : await Response<GenericResponse>.GetErrorResponseMessage(
        ...
        )
        {
            ...
            {
                return await Response<GenericResponse>.GetErrorResponseMessage(
                    "There is a user account with the provided email Or Its not a valid Email"
                );
            }
            ...
            var newUser = CreateNewUnverifiedUserObject(
                ...
                email,
                phone,
                new HashingManager().HashToString(password),
                role,
                new Helper().TokenToHash(token),
                createdBy,
                createdOn
            );
            ...
            if (Id != -1)
            {
                if (newUser.Role == Roles.Customer)
                {
                    return await _emailService.SendVerificationEmail(
                        ConstVariables.EmailVerificationBody,
                        newUser.Email,
                ...
                else
                {
                    ...
                    return await _emailService.SendVerificationEmail(
                        ConstVariables.NewAccountConformationBody,
                        newUser.Email,
        ...
        public async Task<IResult<GenericResponse>> ForgotPassword(string email)
        {
            ...
            if (!user.IsNone)
            {
                ...
                {
                    return await _emailService.SendVerificationEmail(
                        ConstVariables.ResetPasswordEmailBody,
                        email,
                        "Reset your password",
                        true,
                        resetToken,
                        alwaysAUser.Role == Roles.Customer
                    );
                }
                return await Response<GenericResponse>.GetErrorResponseMessage(
                    "Problem creating the reset token."
                );
            }
            return await Response<GenericResponse>.GetErrorResponseMessage(
        ...
        public async Task<IResult<GenericResponse>> ResetPasswordWithResetToken(
            string newPassword,
            string resetToken
        )
        {
            var user = await _userService.GetUserBasedOnResetToken(
                new Helper().TokenToHash(resetToken)
            );
            ...
            if (!user.IsNone)
            {
                ...
                if (_userService.UpdateUser(alwaysAUser, alwaysAUser.Id))
                {
                    return await Task<IResult<GenericResponse>>.Factory.StartNew(
                        () => Response<GenericResponse>.SuccessResponese("Password is updated")
                    );
                }
                return await Response<GenericResponse>.GetErrorResponseMessage(
                    "Problem resetting new password"
                );
            }
            return await Response<GenericResponse>.GetErrorResponseMessage(
            ...
            return claimOption.IsSome
                ? _userService.InsertExpiredKey(claimOption.Always().Value)
                    ? Task<IResult<GenericResponse>>.Factory.StartNew(
                        () => Response<GenericResponse>.SuccessResponese("Successfully logged out.")
                    )
        ...
        public async Task<IResult<GenericResponse>> UpdatePassword(
            IOption<long> id,
        ...
        )
        {
            ...
            if (!user.IsNone)
            {
                ...
                alwaysAUser.Password = new HashingManager().HashToString(
        ...
        public async Task<IResult<GenericResponse>> UpdateCustomerProfile(
            IOption<long> id,
        ...
        public async Task<IResult<GenericResponse>> AddOrUpdateAddress(
            IOption<long> id,
        ...
        )
        {
            ...
            if (addressData.IsAddOperation)
            {
                ...
                var ret = await _userService.AddOrUpdateAddress(
                    addressToCreate,
                    addressData.IsAddOperation
                );
        ...
        public async Task<IResult<IEnumerable<UserAddress>>> GetAddress(
            IOption<long> id,
            long addressId,
            byte addressType,
            long requestUserId
        )
        {
            if (id.IsNone || id.Always() != requestUserId)
            {
                return await Response<IEnumerable<UserAddress>>.GetErrorResponseMessage(
                    "You are not authorized."
                );
            }
        ...
        public async Task<IResult<GenericResponse>> DeleteAddress(
            IOption<long> id,
            long addressId,
            long requestUserId
        )
        ...
        public async Task<IResult<IEnumerable<UserProfile>>> GetUserProfile(
            IOption<long> id,
            long userId,
            long requestUserId
        )
        {
            if (id.IsNone || id.Always() != requestUserId)
            {
                return await Response<IEnumerable<UserProfile>>.GetErrorResponseMessage(
                    "You are not authorized."
                );
            }
            ...
            return (await _userService.GetUserProfile(userId))
                .Map(
                    userProfile => Response<IEnumerable<UserProfile>>.SuccessResponese(userProfile)
                )
                .Or(Response<IEnumerable<UserProfile>>.ErrorResponse("User is not found"));
        }

        public async Task<IResult<GenericResponse>> UpdateUserProfile(
            IOption<long> id,
            long userId,
        ...
        public async Task<IResult<GenericResponse>> DeleteUser(
            IOption<long> id,
            long userId,
            long requestUserId
        )
        {
            ...
            return code == 0
                ? await Task<IResult<GenericResponse>>.Factory.StartNew(
                    () =>
                        Response<GenericResponse>.SuccessResponese("Successfully deleted the user.")
                )
                : await Response<GenericResponse>.GetErrorResponseMessage(
                    ret?.ReturnMsg?.ToString()
                );
        ...
        public async Task<IResult<GenericResponse>> UpdateUserTheme(
            IOption<long> id,
            long userId,
            string theme,
            long requestUserId
        ...
        private string JwtTokenGen(User user)
        {
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                ...
                Expires = DateTime.UtcNow.AddMinutes(
                    expiryDuration.IsSome ? expiryDuration.Always() : 120
                ),
