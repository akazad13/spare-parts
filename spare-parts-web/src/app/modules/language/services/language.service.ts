...
export class LanguageService {
  ...
  get current(): Language {
    return (
      this.languages.find((x) => x.code === this.translate.currentLang) ||
      this.default
    );
  }
  ...
  init(defaultLang: string, userLangs: string[]): Promise<void> {
    ...
    userLangs = userLangs || [];
    userLangs = userLangs.reduce(
      (acc, code) => [...acc, code.split('-').shift()],
      userLangs
    );
    ...
    return this.translate
      .use(storedLang || userLang || defaultLang)
      .toPromise();
  }
