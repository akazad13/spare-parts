...
namespace DHP.AppService.Services
{
    public class UserService : Helper, IUserService
    {
        ...
        {
            return IsValidEmail(email) && (await _userRepository.GetUserBasedOnEmail(email))
        ...
        {
            return (await _userRepository.GetUserBasedOnResetToken(resetToken)).Then(Option.FromMaybeNull);
        }
        ...
        {
            return DoUnitOfWork((string emailAddress) =>
              new System.Net.Mail.MailAddress(emailAddress).Address == emailAddress, email, () => false
            );
        }
        ...
        public async Task<IOption<IEnumerable<UserAddress>>> GetAddress(long userId, long id, byte addressType)
        {
            return (await _userRepository.GetUserAddress(userId, id, addressType)).Then(Option.FromMaybeNull);
        }
