...
namespace DHP.Api.Controllers
{
    ...
    public class CustomerController : BaseController
    {
        ...
        {
            return (await _accountService.LogInService(loginData, true))
                  .Match<IActionResult>(
                        Ok,
                        BadRequest
                   );
        ...
        {
            return (await _accountService.RegisterCustomer(registerData))
                   .Match<IActionResult>(
                        Ok,
                        BadRequest
                   );
        ...
        public IActionResult UpdateProfile(UpdateCustomerProfileDto updateProfileData, long userId)
        {
            return DoWorkAfterValidation(
                         () => _accountService.UpdateCustomerProfile(GetCurrentUserId(), updateProfileData, userId).Result
                            .Match<IActionResult>(
                                Ok,
                                BadRequest
                            )
        ...
        {
            return DoWorkAfterValidation(
                         () => _accountService.AddOrUpdateAddress(GetCurrentUserId(), addressData, userId).Result
                            .Match<IActionResult>(
                                Ok,
                                BadRequest
                            )
        ...
        public IActionResult GetAddress(long userId, long id, byte type)
        {
            ...
            return DoWorkAfterValidation(
                         () => _accountService.GetAddress(GetCurrentUserId(), id, type, userId).Result
                            .Match<IActionResult>(
                                Ok,
                                BadRequest
                            )
        ...
        {
            return DoWorkAfterValidation(
                         () => _accountService.DeleteAddress(GetCurrentUserId(), id, userId).Result
                            .Match<IActionResult>(
                                Ok,
                                BadRequest
                            )
