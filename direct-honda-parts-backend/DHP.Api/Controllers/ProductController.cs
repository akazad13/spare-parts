...
namespace DHP.Api.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        {
            return (await _productService.GetModels())
                   .Match<IActionResult>(
                       Ok,
                       BadRequest
                   );
        ...
        {
            return (await _productService.GetYearsByModel(modelId))
                   .Match<IActionResult>(
                       Ok,
                       BadRequest
                   );
        ...
        {
            return (await _productService.GetBodyAndTrims(modelId, yearId))
                   .Match<IActionResult>(
                       Ok,
                       BadRequest
                   );
        ...
        [HttpGet]
        [Route("models/{modelId}/years/{yearId}/door/{doorId}/grade/{gradeid}/emissionAndTransmission")]
        public async Task<IActionResult> GetEmissionAndTransmission(
            int modelId, int yearId, int doorId, int gradeId)
        {
            return (await _productService.GetEmissionAndTransmission(
                modelId, yearId, doorId, gradeId))
                   .Match<IActionResult>(
                       Ok,
                       BadRequest
                   );
